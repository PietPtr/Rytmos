...
impl Music {
        ...
        D: DrawTarget<Color = BinaryColor>,
    {
        if symbol.symbol.rest {
            ...
            if !beamed {
                // Draw flag
                if symbol.symbol.stem_direction == StemDirection::Up {
                    let pos = position
                        + Point {
                            x: 6,
                    ...
                    match symbol.symbol.kind {
                        Duration::Eighth => {
                            crate::symbols::draw_symbol(target, pos, EIGHT_FLAG)?;
                        }
                        Duration::DottedEighth => {
                            crate::symbols::draw_symbol(target, pos, DOTTED_EIGHTH_REST)?;
                        }
                        Duration::Sixteenth => {
                            crate::symbols::draw_symbol(target, pos, SIXTEENTH_FLAG)?;
                        }
                        _ => {}
                    }
                } else {
...
impl Note {
    const STEM_DIRECTION_SWITCH_HEIGHT: i32 = 19;

    fn y_offset(self) -> i32 {
        match self {
            Note::A(_, octave) => 40 - octave * 14,
            Note::B(_, octave) => 38 - octave * 14,
            Note::C(_, octave) => 50 - octave * 14,
            Note::D(_, octave) => 48 - octave * 14,
            Note::E(_, octave) => 46 - octave * 14,
            Note::F(_, octave) => 44 - octave * 14,
            Note::G(_, octave) => 42 - octave * 14,
        }
    }
